...
- 默认要求上一段保存后的 MCAP 在 **100–145 MB** 范围内（包含边界）。若小于 100 MB 或大于 145 MB 且尚未删除，按 `4` 不会开始下一段；recorder 会同时发布 `/ros2recordstop` 和 `/fsm_command=2`，命令机器人回到 `HOLD`、退出/取消 OCS2 遥操。终端会提示先按 `13`（或输入 `d/discard`）删除该段。
- 上一段仍在后台保存时，按 `4` 也会被暂时拦截，避免文件尚未写完就误判大小；看到“保存完成”后可重试。
- 位于 100–145 MB 范围内时可直接开始下一段；一旦下一段开始，上一段就不能再撤销。上下限可通过 `minimum_episode_size_mb:=100.0` 和 `maximum_episode_size_mb:=145.0` 调整，设为 `0` 可分别关闭对应检查。
